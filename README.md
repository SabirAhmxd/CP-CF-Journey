...
* **The Takeaway:** Learned to completely avoid an $O(n \cdot q)$ Time Limit Exceeded (TLE) brute force. By precomputing a 1-indexed prefix sum array of size `n + 1`, any subarray range sum can be calculated instantly in constant time via:
  $$\text{range\_sum} = \text{pre}[r] - \text{pre}[l - 1]$$
